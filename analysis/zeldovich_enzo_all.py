...
outDir = dev_dir + 'figures/zeldovich/error_{2}_{1}_{0}_eta{4:.3f}{3}/'.format( RECONSTRUCTION, RIEMANN, INTEGRATOR, extra_name, eta_2)
if rank == 0:
...
vel_x_ch = data_cholla['gas']['momentum_x'][...] / dens_ch
vel_y_ch = data_cholla['gas']['momentum_y'][...] / dens_ch
vel_z_ch = data_cholla['gas']['momentum_z'][...] / dens_ch
E_ch = data_cholla['gas']['Energy'][...]
U_ch = data_cholla['gas']['GasEnergy'][...]
potential = data_cholla['gas']['potential'][...]
Ekin_ch = 0.5 * dens_ch * ( vel_x_ch*vel_x_ch + vel_y_ch*vel_y_ch + vel_z_ch*vel_z_ch)
temp_ch = get_temp(U_ch / dens_ch * 1e6, mu=1)
data_ch = [ dens_ch, vel_x_ch, temp_ch, U_ch, E_ch, Ekin_ch ]
data_cholla_all.append(data_ch)
...
for j_indx in range(32):
  for i_indx in range(32): 
    ax_list[0].plot( x, dens_ch[:, j_indx, i_indx] )
    ax_list[1].plot( x, vel_x_ch[:, j_indx, i_indx] )
    ax_list[2].plot( x, temp_ch[:, j_indx, i_indx] )
    ax_list[3].plot( x, vel_y_ch[:, j_indx, i_indx] )
    ax_list[4].plot( x, vel_z_ch[:, j_indx, i_indx] )
    ax_list[5].plot( x, potential[:, j_indx, i_indx] )
