...
n_cholla_files = 1
chollaDir_0 = dataDir + 'cosmo_sims/cholla_pm/zeldovich/data_PLMC_HLLC_VL_eta0.001_0.030/'
# chollaDir_1 = dataDir + 'cosmo_sims/cholla_pm/zeldovich/data_PPMP_HLLC_VL_eta0.001_0.030_z1/'
chollaDir_1 = dataDir + 'cosmo_sims/cholla_pm/zeldovich/data_PLMC_HLLC_VL_eta0.001_0.030_z1/'
chollaDir_2 = dataDir + 'cosmo_sims/cholla_pm/zeldovich/data_PLMP_HLLC_VL_eta0.001_0.030_z1/'


chollaDir_all = [ chollaDir_0, chollaDir_1, chollaDir_2, chollaDir_2]
cholla_label_all = [r'Cholla PPMC', r'Cholla PLMC',  r'Cholla PLMP' ]


enzoDir = dataDir + 'cosmo_sims/enzo/ZeldovichPancake_HLLC/'


outDir = dev_dir + 'figures/zeldovich_PLMC_GOOD/'
if rank == 0:
...
file_name = enzoDir + 'DD{0:04}/data{0:04}'.format(nSnap+startSnap)
...
for j in range(32):
  for i in range(32):
    d = data_cholla['gas']['density'][...][:, j, i]
    vx = data_cholla['gas']['momentum_x'][...][:, j, i] / d
    vy = data_cholla['gas']['momentum_y'][...][:, j, i] / d
    vz = data_cholla['gas']['momentum_z'][...][:, j, i] / d
    E = data_cholla['gas']['Energy'][...][:, j, i]
    U = data_cholla['gas']['GasEnergy'][...][:, j, i]
    temp = get_temp(U / d * 1e6, mu=1)
    ax = ax_list[0]
    ax.plot( x, d )
