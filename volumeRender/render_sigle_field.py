...
inDir = dataDir + 'cosmo_sims/cholla_pm/sphere_explosion/data/'
outDir = '/home/bruno/Desktop/'

# #Cholla fie names
data_base_name = 'grid'

dataFiles = [f for f in listdir(inDir) if (isfile(join(inDir, f)) and (f.find(data_base_name) >= 0 ) ) ]
...
def load_data_snap( nSnap):
  file_name = inDir + data_base_name + '_{0}.h5'.format( nSnap)
