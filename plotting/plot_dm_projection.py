...
chollaDir = dataDir + 'cosmo_sims/cholla_pm/256_dm_100Mpc/'.format(nPoints)
chollaDir_uv = chollaDir +  'data/'

nyxDir = dataDir + 'cosmo_sims/nyx/256_dm_100Mpc/'

outDir = dev_dir + 'figures/dm_projection_100Mpc_Good/'
if rank == 0:
...
for nSnap in range(323):
  ...
  ax = ax_list[0]
  im = ax.imshow( proj_en, interpolation='bilinear',  vmin=min_val, vmax=max_val, cmap=SCM5.davos )
  ...
  ax = ax_list[1]
  im = ax.imshow( proj_ch, interpolation='bilinear',  vmin=min_val, vmax=max_val, cmap=SCM5.davos )
